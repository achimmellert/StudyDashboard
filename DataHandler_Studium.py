...
class Module:
    ...
    def get_progress(self) -> int:  # Getter-Methoden (Kapselung)
...
class Semester:
    ...
    def add_modul(self, modul: Module):
...
class Studies:
    ...
    def __new__(cls):  # Singleton: Es gibt nur eine einzige Instanz dieser Klasse
        if cls._instance is None:
            cls._instance = super(Studies, cls).__new__(cls)
    ...
    def get_overall_progress(self) -> int:  # Polymorphismus
        overall_modules = 0
        overall_progress = 0
        for sem_obj in self.semester_dict.values():
